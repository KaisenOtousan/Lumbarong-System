...
class CheckoutController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('productId')) {
            ...
            $images = is_string($product->image) ? json_decode($product->image, true) : (is_array($product->image) ? $product->image : [$product->image]);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($mode === 'cart') {
                session()->forget('cart');
                if (Auth::check()) {
                    Auth::user()->update(['cart' => json_encode([])]);
                }
            } elseif ($mode === 'selected') {
                ...
                if (Auth::check()) {
                    Auth::user()->update(['cart' => json_encode($mainCart)]);
                }
            } else {
