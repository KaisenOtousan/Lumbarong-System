...
@section('content')
    <div class="max-w-[900px] mx-auto space-y-8">
        ...
        <form action="{{ route('seller.profile.update') }}" method="POST" enctype="multipart/form-data"
            ...
            @method('PUT')
            {{-- Profile Header --}}
            <div class="p-8 flex items-center gap-6 transition-all duration-500 @if($user->isPremiumActive()) border-b border-yellow-500/20 @endif"
                style="background: @if($user->isPremiumActive()) linear-gradient(to right, #1A1816, #2D261E, #8C6D3F) @else linear-gradient(to right, #3D2B1F, #C0420A) @endif;">
                <div class="relative group w-20 h-20 shrink-0" x-data="{ 
                    ...
                    <input type="file" name="profilePhoto"
                        ...
                        ">
                    <div class="w-full h-full rounded-2xl bg-white/20 border-2 flex items-center justify-center text-white font-black text-2xl overflow-hidden relative group-hover:border-white transition-all"
                        style="border-color: {{ $user->isPremiumActive() ? '#facc15' : 'rgba(255, 255, 255, 0.3)' }};">
                ...
                <div>
                    <div class="text-white font-bold text-lg flex items-center gap-2">
                        ...
                        @if($user->isPremiumActive())
                            <span class="text-yellow-400 text-sm" title="Premium Seller">👑</span>
                        @endif
                    </div>
                    ...
                    <div class="mt-2 flex items-center gap-2 flex-wrap">
                        ...
                        @if($user->isPremiumActive())
                            <span
                                class="px-2 py-0.5 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 rounded text-[9px] font-black uppercase tracking-widest">👑
                                Premium Seller</span>
                        @endif
                    </div>
                </div>
            ...
            <div class="p-8 space-y-8">
                ...
                @if($user->isPremiumActive())
                    <div class="p-6 border border-yellow-500/20 rounded-3xl text-white shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden"
                        style="background: linear-gradient(to right, #1A1816, #2D261E, #604E35);">
                        <div class="absolute -right-10 -top-10 w-28 h-28 bg-yellow-500/5 rounded-full blur-2xl"></div>
                        <div class="space-y-1 relative z-10">
                            <div class="text-[9px] font-black text-yellow-400 uppercase tracking-widest">Active Plan</div>
                            <h4 class="font-serif text-lg font-bold flex items-center gap-1.5">Premium Artisan Membership <span
                                    class="text-sm">👑</span></h4>
                            <p class="text-[11px] text-stone-300">Unlimited product listings, premium storefront badge, priority
                                search sorting and promoting banner request.</p>
                        </div>
                        <div
                            class="text-left sm:text-right shrink-0 relative z-10 border-t sm:border-t-0 border-white/10 pt-3 sm:pt-0">
                            <div class="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Valid Until</div>
                            <div class="text-sm font-black text-yellow-400 mt-0.5">
                                {{ $user->premiumEndsAt ? $user->premiumEndsAt->format('F d, Y') : 'N/A' }}</div>
                        </div>
                    </div>
                @endif
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group"
                                    ...
                                    <input type="file" name="gcashQrCode"
                                        ...
                                        ">
                                    <div
                                        class="border-2 border-dashed border-gray-200 bg-white rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-blue-500 hover:bg-blue-50/5 transition-all min-h-[140px]">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group"
                                    ...
                                    <input type="file" name="mayaQrCode"
                                        ...
                                        ">
                                    <div
                                        class="border-2 border-dashed border-gray-200 bg-white rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:border-green-500 hover:bg-green-50/5 transition-all min-h-[140px]">
                                        <template x-if="qrPreview">
                                            <div class="relative w-28 h-28">
                                                <img :src="qrPreview" class="w-full h-full object-contain rounded-lg">
                                                <div
